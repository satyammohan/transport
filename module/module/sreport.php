...
class sreport extends common {
    ...
    function representper() {
        ...
        if ($company) {
            $sql = "SELECT s.id_represent, s.date, count(*) AS cnt, SUM(s.total) AS total, r.name AS rname, s.id_area, a.name
            FROM {$this->prefix}represent r,  {$this->prefix}sale s LEFT JOIN {$this->prefix}area a ON s.id_area=a.id_area
            WHERE s.id_represent=r.id_represent AND s.date>='$sdate' AND s.date<='$edate' AND s.id_company='$company'
            GROUP BY rname, s.date, s.id_area
            ORDER BY rname, s.date, a.name";
	$sql = "SELECT id_represent, date, count(*) AS cnt, SUM(total) AS total, rname, id_area, name FROM
        ( SELECT s.id_represent, s.date, count(*) AS cnt, SUM(s.total) AS total, r.name AS rname, s.id_area, a.name
            FROM {$this->prefix}represent r,  {$this->prefix}sale s LEFT JOIN {$this->prefix}area a ON s.id_area=a.id_area
            WHERE s.id_represent=r.id_represent AND s.date>='$sdate' AND s.date<='$edate' AND s.id_company='$company'
            GROUP BY rname, s.date, s.id_area
        UNION ALL
          SELECT s.id_represent, s.date, count(*) AS cnt, SUM(-s.total) AS total, r.name AS rname, s.id_area, a.name
            FROM {$this->prefix}represent r,  {$this->prefix}creditnote s LEFT JOIN {$this->prefix}area a ON s.id_area=a.id_area
            WHERE s.id_represent=r.id_represent AND s.date>='$sdate' AND s.date<='$edate' AND s.id_company='$company' AND s.saletype='1'
            GROUP BY rname, s.date, s.id_area) a GROUP BY rname, date, id_area
            ORDER BY rname, date, name";
    ...
    function representInvoiceDetail() {
        ...
        $sql = "SELECT r.name AS rname, sd.invno, sd.date, h.name AS pname, h.address1, p.name, sd.*, s.tcsper 
            FROM `{$this->prefix}product` p, `{$this->prefix}represent` r, `{$this->prefix}sale` s,  `{$this->prefix}saledetail` sd LEFT JOIN `{$this->prefix}head` h ON sd.id_head=h.id_head
    ...
    function gst_return() {
		...
		break;
        	case 4:
	        $sql = "SELECT s.id_sreturn, s.slno, s.date, h.name, s.challan_no, s.challan_date, s.total, h.gstin, h.state, 
	                 sd.tax_per, SUM(sd.goods_amount) AS tax_amount, SUM(sd.cessamt) AS cessamt
        	    	FROM `{$this->prefix}sreturndetail` sd, `{$this->prefix}taxmaster` m, `{$this->prefix}sreturn` s 
                		LEFT JOIN `{$this->prefix}head` h ON h.id_head=s.id_head 
	        	WHERE ($wcond) AND sd.id_taxmaster=m.id_taxmaster AND s.id_sreturn=sd.id_sreturn
	            	GROUP BY s.id_sreturn, sd.tax_per ORDER BY s.date, s.slno, sd.tax_per ";
        }
        $res = $this->m->sql_getall($sql);
        $this->sm->assign("data", $res);
    ...
    function gst() {
		...
		case 1:
			...
			$sql = "SELECT s.id_sale, s.invno, s.date, s.party_name, s.party_address, s.totalamt, s.vat, s.totalcess, s.add, s.less,
				s.round, s.packing, s.total, h.local, h.gstin, s.discount, s.tcsamt 
		...
		case 2:
			...
			$sql = "SELECT s.date, SUM(s.totalamt) AS totalamt, SUM(s.vat) AS vat, SUM(s.total) AS `total`,
				SUM(s.add) AS `add`,  SUM(s.less) AS less, SUM(s.round) AS round, SUM(s.packing) AS packing, SUM(s.tcsamt) AS tcsamt
				FROM `{$this->prefix}sale` s LEFT JOIN `{$this->prefix}head` h ON h.id_head=s.id_head
				WHERE $wcond $tcond  GROUP BY s.date ORDER BY s.date ";
			break;
		case 3:
...
// goods_amount is changed to amount
	...
	//$wcond = isset($_REQUEST['debug']) ? " HAVING tax_amount!=vat OR total!=net_amount+other " : "";
        $sql = "SELECT s.id_sale, s.invno, s.date, s.vat, s.packing+s.add-s.less+s.round+s.tcsamt AS other, s.total, SUM(sd.amount) AS amount, 
            ...
            FROM `{$this->prefix}sale` s LEFT JOIN `{$this->prefix}saledetail` sd ON s.id_sale=sd.id_sale
            WHERE s.date >= '$sdate' AND s.date <= '$edate'
            GROUP BY s.id_sale $wcond ORDER BY s.date, s.invno";
        $res = $this->m->sql_getall($sql);
        $this->sm->assign("data", $res);
    }
    ...
    function represent() {
        ...
        switch ($_REQUEST['option']) {
            ...
            case 4:
                ...
                break;
            case 2:
                $sql = "SELECT s.date, r.name AS rname, a.name AS aname, SUM(s.vat+s.totalcess+s.tcsamt) AS vat, SUM(s.total) AS `total`, COUNT(s.invno) AS bills, s.id_represent, s.id_sale
                ...
                break;
            case 3:
                $sql = "SELECT r.name AS rname, SUM(s.vat+s.totalcess+s.tcsamt) AS vat, SUM(s.total) AS `total`, COUNT(s.invno) AS bills, s.id_represent
...
/* $sql = "SELECT a.rname, SUM(a.vat) AS vat, SUM(a.totalcess) AS totalcess, SUM(a.total) AS `total`, COUNT(a.bills) AS bills, a.id_represent
 FROM ( SELECT r.name AS rname, SUM(s.vat) AS vat, SUM(totalcess) AS totalcess, SUM(s.total) AS `total`, COUNT(s.invno) AS bills, s.id_represent
    FROM {$this->prefix}sale s, {$this->prefix}represent r
    WHERE s.id_represent=r.id_represent AND s.date>='$sdate' AND s.date<='$edate' {$wcond} GROUP BY s.id_represent
 UNION ALL
   SELECT r.name AS rname, SUM(-s.vat) AS vat, SUM(-totalcess) AS totalcess, SUM(-s.total) AS `total`, COUNT(s.no) AS bills, s.id_represent
    FROM {$this->prefix}creditnote s, {$this->prefix}represent r
    WHERE s.id_represent=r.id_represent AND s.date>='$sdate' AND s.date<='$edate' {$wcond} GROUP BY s.id_represent) a
 GROUP BY a.id_represent ORDER BY a.rname ";*/
	    ...
	    if ($_REQUEST['option']==4) {
               $sql = "SELECT s.id_product, r.name AS rname, s.id_represent, p.name, p.case, p.weight, s.rate, SUM(s.qty) AS qty, SUM(s.free) AS free, SUM(s.amount) AS amount
                    FROM {$this->prefix}saledetail s, {$this->prefix}product p, {$this->prefix}represent r 
                    WHERE s.id_product=p.id_product AND s.id_represent=r.id_represent AND s.date>='$sdate' AND s.date<='$edate' " . " {$wcond} GROUP BY s.id_represent, s.id_product ORDER BY s.id_represent, p.name ";
	    } else {
    ...
    function itemrep() {
        ...
        $wcond = '';
        if ($id_company != 0) {
            $wcond .= " AND sd.id_company='$id_company' ";
    ...
    function partyitem() {
        ...
        $sql = "SELECT concat(s.party_name,' ', s.party_address) AS party, p.discounted_seller_price, p.name AS iname,  p.short, 
                 MAX(sd.rate) AS rate, SUM(sd.qty) AS qty, SUM(sd.free) AS free, SUM(sd.amount) AS amount, p.case, 
		 SUM(sd.tax_amount) AS tax_amount, SUM(net_amount) AS net_amount
                FROM {$this->prefix}sale s, {$this->prefix}saledetail sd, {$this->prefix}product p
    ...
    function party() {
        ...
        $id_rep = isset($_REQUEST['salesman']) ? $_REQUEST['salesman'] : '0';
        if ($id_rep != 0) {
            $wcond .= "AND s.id_represent='$id_rep'";
	}
        $id_company = isset($_REQUEST['company']) ? $_REQUEST['company'] : '0';
        if ($id_company != 0) {
            $wcond .= "AND s.id_company='$id_company'";
        ...
        switch ($_REQUEST['option']) {
            ...
            case 4:
                $sql_detail = "SELECT s.*, p.name FROM `{$this->prefix}saledetail` s, `{$this->prefix}product` p
                    WHERE s.id_product=p.id_product AND `date` >= '$sdate' AND `date` <= '$edate' ORDER BY id_sale, p.name";
                $detail = $this->m->sql_getall($sql_detail, 1, "", "id_sale", "id_saledetail");
                $this->sm->assign("detail", $detail);
                $sql = "SELECT s.id_sale, s.id_head, s.invno, s.date, s.vat, s.totalcess, s.total, 
                        s.party_name AS party, s.party_address, s.party_address1 FROM {$this->prefix}sale s
                        WHERE s.date >= '$sdate' AND s.date <= '$edate' {$wcond} ORDER BY party, s.date  ";
                $res = $this->m->sql_getall($sql, 1, "", "id_head", "id_sale");
                break;
        }
