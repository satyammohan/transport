...
class debitnote extends common {
    ...
    function check() {
        ...
        $sql = $this->create_select("{$this->prefix}debitnote", "no='$no' AND `date`>='$sdate' AND `date`<='$edate'");
    ...
    function edit() {
        ...
        if ($id == 0) {
            $this->get_permission("debitnote", "INSERT");
			$sql = "SELECT MAX(CAST(no AS UNSIGNED)) AS maxid FROM {$this->prefix}debitnote";
			$data = $this->m->fetch_assoc($sql);
			$data['no'] = $data['maxid'] + 1;
			$this->sm->assign("sdata", $data);
		} else {
            $this->get_permission("debitnote", "UPDATE");
			$sql = "SELECT s.*, p.name AS item, p.case AS itemcase FROM {$this->prefix}debitnotedetail s, {$this->prefix}product p WHERE s.id_product=p.id_product AND s.id_debitnote='$id' ORDER BY id_debitnotedetail";
			$this->sm->assign("data", $this->m->sql_getall($sql));
			$sql = "SELECT s.*, concat(h.name, ',', h.address1, ',', h.address2, ',', IFNULL(address3, '')) AS name, h.address1, h.address2, h.gstin, h.dealer FROM {$this->prefix}debitnote s LEFT JOIN {$this->prefix}head h ON s.id_head=h.id_head WHERE s.id_debitnote='$id' ";
        ...
        $this->option_val();
        $this->sm->assign("page", "debitnote/add.tpl.html");
    }

    function prsale($id = "") {
        unset($_SESSION['url']);
        $this->get_permission("debitnote", "REPORT");
        ...
        $sql = "SELECT s.*, h.pincode, h.phone, h.mobile, h.local, h.gstin, h.panno, h.adhar, h.name AS hname, h.address1,  h.address2, h.address3, h.email
                FROM {$this->prefix}debitnote s
                LEFT JOIN {$this->prefix}head h ON s.id_head=h.id_head
                WHERE s.id_debitnote IN ($id) GROUP BY s.id_debitnote ";
        $res1 = $this->m->sql_getall($sql);
        foreach ($res1 as $key => $val) {
            $res1[$key]['w'] = $this->convert_number(round($val['total']));
        }
        $this->sm->assign("debitnote", $res1);
        $sql = "SELECT s.*, b.expiry_date, p.name AS item, p.hsncode, p.case 
		FROM {$this->prefix}debitnotedetail s LEFT JOIN {$this->prefix}batch b ON s.id_batch=b.id_batch, {$this->prefix}product p 
		WHERE s.id_product=p.id_product AND s.id_debitnote IN ($id)";
        $res = $this->m->sql_getall($sql, 1, "", "id_debitnote", "id_debitnotedetail");

        $this->sm->assign("debitnotedetail", $res);
        ...
        $this->sm->assign("page", "debitnote/print.tpl.html");
    }

    function insert() {

        $this->get_permission("debitnote", "INSERT");
        $data1 = $_REQUEST['debitnote'];
        ...
        $sql = $this->create_insert("{$this->prefix}debitnote", $data1);
        $sql2 = $this->m->query($sql);
        $id = $this->m->getinsertID($sql2);
        $this->insertdetail($data1, $id);
        $_SESSION['msg'] = "Debit Note Successfully Inserted";
        if (isset($_REQUEST['ce'])) {
            $this->prsale($id);
        } else {
            $this->redirect("index.php?module=debitnote&func=listing");
        }
    }
    ...
    function insertdetail($data1, $id) {
        for ($i = 0; $i < count($_REQUEST['items']); $i++) {
		    ...
		    "saletype" => "{$data1['saletype']}", "mode" => "{$mode}",
                    ...
                    "id_debitnote" => "{$id}", "id_head" => "{$data1['id_head']}", "net_amount" => $net_amount,
                    "id_batch" => "{$_REQUEST['id_batch'][$i]}", "batch_no" => "{$_REQUEST['batch_no'][$i]}", "exp_date" => "{$_REQUEST['exp_date'][$i]}");
                $sql = $this->create_insert("{$this->prefix}debitnotedetail", $data);
                $this->m->query($sql);
            }
        }
    }
    ...
    function update() {
        ...
        $data1['refdate'] = $this->format_date($data1['refdate']);
        $data1['billdate'] = $this->format_date($data1['billdate']);
        $this->m->query($this->create_update("{$this->prefix}debitnote", $data1, "id_debitnote='$id'"));
        $this->m->query($this->create_delete("{$this->prefix}debitnotedetail", "id_debitnote='$id'"));
		$this->insertdetail($data1, $id);
        $_SESSION['msg'] = "Debit Note Successfully Updated";
        if (isset($_REQUEST['ce'])) {
            $this->prsale($id);
        } else {
            $this->redirect("index.php?module=debitnote&func=listing");
    ...
    function listing() {
        ...
        $sql = "SELECT s.*, h.name, h.address1, h.address2, h.gstin, r.name AS rname 
            FROM {$this->prefix}debitnote s, {$this->prefix}head h, {$this->prefix}represent r
    ...
    function delete() {
        $this->get_permission("debitnote", "DELETE");
        $this->m->query($this->create_delete("{$this->prefix}debitnote", "id_debitnote='{$_REQUEST['id']}'"));
        $this->m->query($this->create_delete("{$this->prefix}debitnotedetail", "id_debitnote='{$_REQUEST['id']}'"));
        $_SESSION['msg'] = "Debit Note Successfully Deleted";
        $this->redirect("index.php?module=debitnote&func=listing");
    }
