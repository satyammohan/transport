...
class gst extends common {
    ...
    function gstr3b() {
        $sdate = $this->getstartdate();
        $edate = $this->getenddate();
        $sql = "SELECT SUM(sd.totalamt-sd.discount) AS goods_amt, SUM(sd.vat) AS vat, SUM(sd.total) AS total, SUM(sd.tcsamt) AS tcsamt, SUM(`add`-`less`+`round`+packing) AS other 
                FROM `{$this->prefix}sale` sd WHERE sd.date >= '$sdate' AND sd.date <= '$edate'";
        $res = $this->m->sql_getall($sql);
        $this->sm->assign("saletcs", $res);
        $sql = "SELECT SUM(sd.totalamt-sd.discount) AS goods_amt, SUM(sd.vat) AS vat, SUM(sd.total) AS total, SUM(sd.tcsamt) AS tcsamt, SUM(`add`-`less`+`round`+packing) AS other 
                FROM `{$this->prefix}purchase` sd WHERE sd.date >= '$sdate' AND sd.date <= '$edate'";
        $res = $this->m->sql_getall($sql);
...
//        $this->sm->assign("debitnote", $res);
    ...
    function hsn() {
        ...
        if ($_REQUEST['option']==4 || $_REQUEST['option']==2) {
            ...
            if ($_REQUEST['option']==3 || $_REQUEST['option']==5) {
	...
	$cond = $wcond;
        ...
        case 3:
            $sql = "SELECT p.hsncode, $field, t.name AS taxname
                ...
                WHERE $wcond AND s.id_product=p.id_product AND s.id_taxmaster=t.id_taxmaster GROUP BY 1, h.local, s.id_taxmaster ORDER BY 1, 2 ";
            break;
        case 4:
 	   ...
 	   break;
        case 5:
            $sql = "SELECT LEFT(p.hsncode,6) AS hsncode, $field, t.name AS taxname
                FROM {$this->prefix}{$file} s1, `{$this->prefix}product` pi,`{$this->prefix}taxmaster` t, `{$this->prefix}{$file}detail` s LEFT JOIN `{$this->prefix}head` h ON h.id_head=s.id_head 
                WHERE $wcond AND s.id_product=p.id_product AND s.id_taxmaster=t.id_taxmaster GROUP BY 1, h.local, s.id_taxmaster ORDER BY 1, 2 ";
        }
        $res = $this->executesqlnogroup($sql);
        $this->sm->assign("data", $res);
    ...
    function getdoc($month, $year) {
	 ...
	 $sql = "SELECT trim(invno) AS invno, cast(invno AS decimal) AS i FROM `{$this->prefix}sale` WHERE MONTH(date)=$month AND YEAR(date)=$year AND invno!='' ORDER BY 2, invno";
        $res = $this->m->sql_getall($sql);
        $series = array();
        $start = $next = "";
        foreach ($res as $k => $v) {
	...
	$res = "";
        if ($_REQUEST['submit']=="json") {
            foreach ($inv as $k => $v) {
                ...
                $r['TranDtls']['IgstOnIntra']=($v['local']==0)?"N":"Y";
    ...
    function saleseinvoice() {
        $_REQUEST['start_date'] = @$_REQUEST['start_date'] ? $_REQUEST['start_date'] : date("d/m/Y");
        $_REQUEST['end_date'] = @$_REQUEST['end_date'] ? $_REQUEST['end_date'] : date("d/m/Y");
        $sdate = $this->getstartdate();
        $edate = $this->getenddate();
        $wcond = " (s.date>='$sdate' AND s.date<='$edate') AND h.gstin!='' ";
	$sql = "SELECT id_company AS id, name FROM {$this->prefix}company WHERE status=0 ORDER BY name";
        $company = $this->m->sql_getall($sql, 2, "name", "id");
        $this->sm->assign("company", $company);
        $comp = @$_REQUEST['company'];
    ...
    function saleseinvoice1() {
        //$sdate = $this->getstartdate();
        //$wcond = " s.id_sale=sd.id_sale AND s.date='$sdate' AND h.gstin!='' ";
        $wcond = " s.id_sale=sd.id_sale AND h.gstin!='' ";
	...
	$sql = "SELECT id_company AS id, name FROM {$this->prefix}company WHERE status=0 ORDER BY name";
        ...
        if (@$_REQUEST['submit']=="json") {
            foreach ($inv as $k => $v) {
                ...
                $r['ValDtls']['TotInvVal']=round($v['total'],2);
                if ($v['transport_name']) {
                    $r['EwbDtls']['TransId']=$v['transport_id'];
                    $r['EwbDtls']['TransName']=$v['transport_name'];
                    $r['EwbDtls']['TransMode']='1';
                    $r['EwbDtls']['Distance']=$v['distance'];
                    $r['EwbDtls']['TransDocNo']=$v['invno'];
                    $r['EwbDtls']['TransDocDt']=$d;
                    $r['EwbDtls']['VehNo']=$v['vehicle_number'];
                    $r['EwbDtls']['VehType']='R';
                } else {
                    $r['EwbDtls']=null;
                }
                ...
                $r['ItemList'][$i-1]["OthChrg"] = 0;
                $r['ItemList'][$i-1]["TotItemVal"] = round($v['net_amount'],2);
                if ($v['batch_no'] && $v['exp_date'] && $v['exp_date']!="00/0000") {
                    $r['ItemList'][$i-1]["BchDtls"]['Nm'] = $v['batch_no'];
                    if (strlen($v['exp_date'])==10) {
                        $expconvert = $v['exp_date'];
                        $v['exp_date'] = substr($expconvert,5,2)."-".substr($expconvert,0,4);
                    }
                    $r['ItemList'][$i-1]["BchDtls"]['ExpDt'] = "01/".str_replace("-","/",$v['exp_date']);
                    $r['ItemList'][$i-1]["BchDtls"]['WrDt'] = null;
                } else {
                    $r['ItemList'][$i-1]["BchDtls"] = null;
                }
                if ($i==1) {
                    $res[] = $r;
                } else {
            ...
            foreach ($inv as $k => $v) {
                ...
                $r["Country_Code"]="";
                $r["Export_Duty_Amount"]="";
                $r["Trans_ID"]=$v['transport_id'];
                $r["Trans_Mode"]="1";
                $r['Trans_Name']=$v['transport_name'];
                $r["Distance"]=$v['distance'];
                $r["Trans_Doc_No"]=$v['invno'];
                $r["Trans_Doc_Date"]=$d;
    ...
    function bulk_einvoice() {
        $_REQUEST['start_date'] = @$_REQUEST['start_date'] ? $_REQUEST['start_date'] : date("d/m/Y");
        $_REQUEST['end_date'] = @$_REQUEST['end_date'] ? $_REQUEST['end_date'] : date("d/m/Y");
        $sdate = $this->getstartdate();
        $edate = $this->getenddate();
        $wcond = " (s.date >= '$sdate' AND s.date <= '$edate') AND h.gstin!='' ";
        $sql = "SELECT h.*, sd.*, s.*, i.name as iname, i.hsncode 
		FROM `{$this->prefix}head` h, `{$this->prefix}sale` s, `{$this->prefix}saledetail` sd, {$this->prefix}product i
                WHERE $wcond AND h.id_head=s.id_head AND s.id_sale=sd.id_sale AND sd.id_product=i.id_product ORDER BY s.date, s.invno";
        $this->sm->assign("data", $this->m->sql_getall($sql));      
    }
    function bulk_upload_irn() {
        $excelheading = "Sl. No,Ack No,Ack Date,Doc No,Doc Date,Doc Type,Inv Value.,Recipient GSTIN,Status,IRN,SignedQrCOde,Eway Bill No.";
        $this->sm->assign("excelheading", $excelheading);
        if (@$_FILES['irn']) {
            include_once("SimpleXls.php");
            $f = $_FILES['irn'];
            $xlsx = SimpleXLSX::parse($f['tmp_name']);
            if ( $xlsx ) {
                $inserted = 0;
                $rows = $xlsx->rows();
                $heading = $rows[0];
                $h = implode(",", $heading);
                if ($h!=$excelheading) {
                    $_SESSION['msg'] = "Invalid excel file ".$f['name'].". Please check the Format of the excel file.";
                    $this->redirect("index.php?module=gst&func=bulk_upload_irn");
                }
                $len = count($rows);
                for ($i=1; $i<=$len; $i++) {
                    $currow = $rows[$i];
                    $invno = $currow['3'];
		    $doctype = $currow['5'];
                    $invdate = $this->format_date($currow['4']);
                    $ack = $currow['1'];
                    $ackdate = $currow['2'];
                    $status = $currow['8'];
                    $irn = $currow['9'];
                    $qrcode = $currow['10'];
                    $ewaybill = $currow['11'];
		    $tbl = ($doctype=="Credit Note") ? "sreturn" : "sale";
		    $fld = ($doctype=="Credit Note") ? "slno" : "invno";

                    if ($invno) {
                        $sql = "UPDATE {$this->prefix}{$tbl} SET ack='$ack', ackdate='$ackdate', irn='$irn', qrcode='$qrcode', wbdetails='$ewaybill' WHERE $fld='$invno' AND date='$invdate'";
                        $this->m->query($sql);
                        $inserted++;
                    }
                }
                $_SESSION['msg'] = "$inserted Records updated. Bulk IRN Updated Successfully.";
                $this->redirect("index.php");
            } else {
                $_SESSION['msg'] = "Error reading ".$f['name'].". Upload unsuccessful.";
                $this->redirect("index.php?module=gst&func=bulk_upload_irn");
            }
        }
    }
    function bulk_irn_cancel() {
    }
    function sreturneinvoice() {
        $_REQUEST['start_date'] = @$_REQUEST['start_date'] ? $_REQUEST['start_date'] : date("01/m/Y");
        $_REQUEST['end_date'] = @$_REQUEST['end_date'] ? $_REQUEST['end_date'] : date("d/m/Y");
        $sdate = $this->getstartdate();
        $edate = $this->getenddate();
        $sql = "SELECT h.*, s.*  FROM {$this->prefix}sreturn s, `{$this->prefix}head` h 
                WHERE (s.date>='$sdate' AND s.date<='$edate') AND h.gstin!='' AND h.id_head=s.id_head ORDER BY s.date, s.slno ";
        $inv = $this->m->sql_getall($sql);
        $this->sm->assign("data", $inv);
    }
    function sreturneinvoice1() {
        if (!@$_REQUEST['id']) {
            return;
        }
        $ids = $_REQUEST['id'];
        $wcond = " s.id_sreturn=sd.id_sreturn AND h.gstin!=''  AND s.id_sreturn IN ({$ids}) " ;
        $sql = "SELECT h.*, s.*, sd.*, p.name as iname, p.hsncode, b.expiry_date
            FROM {$this->prefix}sreturn s, `{$this->prefix}head` h , `{$this->prefix}product` p, `{$this->prefix}sreturndetail` sd 
                LEFT JOIN `{$this->prefix}batch` b ON sd.id_batch=b.id_batch
            WHERE $wcond AND sd.id_product=p.id_product AND h.id_head=s.id_head ORDER BY s.date, s.slno ";
        $inv = $this->m->sql_getall($sql);
        $prev = "";
        $l = 1;
	    $res = [];
        foreach ($inv as $k => $v) {
            ...
            $r['ItemList'][$i-1]["CesRt"] = $v['cess']?round($v['cess']):0;
            $r['ItemList'][$i-1]["CesAmt"] = $v['cessamt']?round($v['cessamt'],2):0;
            $r['ItemList'][$i-1]["CesNonAdvlAmt"] = 0;
            $r['ItemList'][$i-1]["StateCesRt"] = 0;
            $r['ItemList'][$i-1]["StateCesAmt"] = 0;
            $r['ItemList'][$i-1]["StateCesNonAdvlAmt"] = 0;
